...
│          (Relations entre variables XML, clusters & pipeline)               │
└─────────────────────────────────────────────────────────────────────────────┘
...
│       │  │   └─ 🔁 Retour ÉTAPE 2 avec micro-ajustement (max 3 cycles → 5⭐)
│       │  └─ SINON (Succès 5⭐ ou demande explicite)
│       │     └─ 📝 Injecte `<response_footer>` (Résumé + Options + Guidance)
│       │
│       └─ PRÉ-FLIGHT CHECK (SELF-CHECK)
│          ├─ ✅ Vérifie séquence 1→2→3→4→5 respectée
│          ├─ ✅ Vérifie ≥3 hacks injectés par instruction
│          ├─ ✅ Vérifie `[À CLARIFIER]` si incertitude (anti-hallucination)
